...
XNLEXPORT xlong  XI_CDECL createQObject(xint type, XObject * x, xlong parent) {
	...
	case None:
		...
		break;
	case qtWidget:
		qobject = new QWidget((QWidget*)parent);
		break;
	case qtMainWindow:
		if (parent != 0) {
			qobject = new QMainWindow((QWidget*)parent);
		}else {
			qobject = new QMainWindow();
		}

		((QMainWindow*)qobject)->window();
		break;
	case qtSci:
		if (parent != 0) {
...
#ifdef WIN32
XNLEXPORT int  __stdcall XNLMain(XNLEnv * env, int version){
#else
XNLEXPORT int  XNLMain(XNLEnv * env, int version){
#endif
	gs_env = env->getEnv();
	return 0;
}
...
#ifdef WIN32
XNLEXPORT int  __stdcall XNLExit(XNLEnv * env){
#else
XNLEXPORT int  XNLExit(XNLEnv * env){
#endif
	if (gs_env != 0) {
		if (_application != 0) {
...
XNLEXPORT XObject * XI_CDECL widget_get_object(xlong h, xint proid) {
	...
	case TREEGETSELITEM:
	{
		...
		while (iter != items.end()) {
			gs_env->setElementValue(objs, id++, (xlong)*iter);
			iter++;
		}
		return objs;
	}
	...
	case TREEGETCHILDREN:
	{
		...
		for (size_t i =0; i < count; i ++){
			gs_env->setElementValue(objs, i, (xlong)((QTreeWidgetItem*)h)->child(i));
		}
		return objs;
	}
		break;
	case SAVESTATE:
	{
		...
		for (size_t i = 0; i < data.size(); i++) {
			gs_env->setElementValue(objs, i, (xbyte)data.at(i));
		}
...
XNLEXPORT xint XI_CDECL widget_set_v2int_double_value(xlong h, xint proid, xint xv, xint yv, double w) {
	switch (proid)
	{
	case SETPEN:
	{
...
XNLEXPORT xbool XI_CDECL array_int2(xlong h, xint proid, XObject * obj, xint pos, xint len) {
	...
	case BUFFERSETDATA:
	{
		...
		if (gs_env->isArray(obj)) {
			...
			for (int i = 0, c = gs_env->getLengthOfArray(obj); i < c; i++) {
				xbyte b;
				gs_env->getElementValue(obj, i, &b);
	...
	case QXPAINTDRAWLINE:
	{
		...
		if (gs_env->isArray(obj)) {
			for (int i = 2, c = gs_env->getLengthOfArray(obj); i < c; i += 2) {
				xint x, y, ex, ey;
				gs_env->getElementValue(obj, i - 2, &x);
				gs_env->getElementValue(obj, i - 1, &y);
				gs_env->getElementValue(obj, i , &ex);
				gs_env->getElementValue(obj, i + 1, &ey);
...
XNLEXPORT XObject * XI_CDECL object_get_handle(xlong h, xint proid, xlong h1) {
	...
	case IMGGETDATA:
	{
		...
		for (size_t i = 0; i < count; i++) {
			gs_env->setElementValue(obj, i, data[i]);
		}
...
XNLEXPORT xlong XI_CDECL long_object_string(xlong handle, xint proid, XObject * v1, xstring v2) {
	...
	case IMGLOAD:
		...
		if (gs_env->isArray(v1)) {
			QByteArray byteArray;
			for (int i = 0, c = gs_env->getLengthOfArray(v1); i < c; i++) {
				xbyte b;
				gs_env->getElementValue(v1, i, &b);
				byteArray.push_back(b);
			}
...
XNLEXPORT void XI_CDECL long_long_int9(xlong h, xint proid, xlong v1, xint x1, xint x2, xint x3, xint x4, xint y1, xint y2, xint y3, xint y4, xint z1) {
	...
	case FILLRECT:
	{
		((QPainter*)h)->fillRect(QRect(x1, x2, x3, x4), QBrush(QColor(y1), Qt::BrushStyle(y2)));
	}
		break;
	}
}
...
XNLEXPORT void XI_CDECL widget_set_intlongint_value(xlong h, xint proid, xlong xv, xint yv, xint zv) {
	...
	{
		QTreeWidgetItem * parent = (QTreeWidgetItem *)xv;
		parent->setTextColor(yv, QColor(zv));
	}
	break;
	...
	{
		QTreeWidgetItem * parent = (QTreeWidgetItem *)xv;
		parent->setBackgroundColor(yv, QColor(zv));
	}
	break;
	case ADDWIDGET:
	{
...
XNLEXPORT void XI_CDECL widget_set_int2_object_value(xlong h, xint proid, xint i, xint iv, XObject * value) {
	...
	case SCICMD:
	{
		if (i == QsciScintilla::SCI_AUTOCSHOW) {
			if (gs_env->isArray(value)) {
				...
				for (xlong i = 0; i < size; i++) {
					const char * text;
					int length;
					if (gs_env->getElementValue(value, i, &text, &length)) {
						wlist.push_back(QString::fromUtf8(text, length));
					}
				}
				wlist.sort();
		...
		case PROPSETENUM:
		{
			...
			if (gs_env->isArray(value)) {
				...
				for (xlong i = 0; i < size; i++) {
					...
					int length;
					if (gs_env->getElementValue(value, i, &text, &length)) {
						columns.push_back(QString::fromUtf8(text, length));
					}
				}

				twidget->setEnumNames((QtProperty*)i, columns);
			}
		}
	}
}
XNLEXPORT void XI_CDECL widget_set_object_value(xlong h, xint proid, XObject * value) {
	...
	case ADDDEFACTION:
	{
		...
		if (gs_env->isArray(value)) {
			...
			for (xlong i = 0; i < size; i++) {
				xlong v;
				if (gs_env->getElementValue(value, i, &v)) {
					actions.push_back((QAction*)v);
					ar.installAction((QAction*)v);
				}
			}
	...
	case PROPATTRENUMN :// 133,
	{
		...
		if (gs_env->isArray(value)) {
			...
			for (xlong i = 0; i < size; i++) {
				...
				int length;
				if (gs_env->getElementValue(value, i, &text, &length)) {
					enumNames.push_back(QString::fromUtf8(text, length));
				}
			}
	...
	case LOADSTATE:// 133,
	{
		...
		if (gs_env->isArray(value)) {
			...
			for (int i = 0; i < size; i++) {
				xbyte bv;
				gs_env->getElementValue(value, i, &bv);
				data.push_back(bv);
			}
...
XNLEXPORT void XI_CDECL native_double4(xlong h, xint proid, double v1, double v2, double v3, double v4) {
	switch (proid)
	{
	case PROPSETARRTRC:
